...
from openprompt.utils.logging import config_experiment_dir, init_logger, save_config_to_yaml
from openprompt.config import get_config
from openprompt.plms import load_plm_from_config
from custom_dataloader import load_dataset
...
def main():
    ...
    # Set up experiment logging and configuration
    ...
    if config.learning_setting == 'full':
        res = trainer(EXP_PATH, config, Processor, train_dataset, valid_dataset, test_dataset)
        ...
        for seed in seeds:
            if not args.test:
                sampler = FewShotSampler(num_examples_per_label=config.sampling_from_train.num_examples_per_label,
                ...
                train_sampled_dataset, valid_sampled_dataset = sampler(train_dataset=train_dataset,
                                                                       valid_dataset=valid_dataset,
                                                                       seed=seed)
                result = trainer(EXP_PATH, config, Processor, train_sampled_dataset, valid_sampled_dataset, test_dataset)
            else:
                result = trainer(EXP_PATH, config, Processor, test_dataset=test_dataset)
            res += result
        res /= len(seeds)
    elif config.learning_setting == 'zero_shot':
        res = trainer(EXP_PATH, config, Processor, train_dataset, valid_dataset, test_dataset, zero=True)


def trainer(EXP_PATH, config, Processor, train_dataset=None, valid_dataset=None, test_dataset=None, zero=False):
    if not os.path.exists(EXP_PATH):
        os.mkdir(EXP_PATH)
    config.logging.path = EXP_PATH
    set_seed(config.reproduce.seed)

    # Load pretrained model, tokenizer, and config
    ...
    # Define the runner and start training/evaluation
